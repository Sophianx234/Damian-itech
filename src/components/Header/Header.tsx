...
const Header = () => {
  ...
  useEffect(() => {
    setMounted(true);
    const storedUser = localStorage.getItem('techNestUser');
    if (storedUser) {
      try {
  ...
  const handleLogout = () => {
    localStorage.removeItem('techNestUser');
    setUser(null);
        ...
        <div className={styles.icons}>
          ...
          <div className={styles.userMenuContainer}>
            ...
            >
              ...
              {user && <span className={styles.userGreeting}>Hi, {user.fullName.split(' ')[0]}</span>}
            </button>
            ...
            {isUserMenuOpen && (
              <div className={styles.userDropdown}>
                {user ? (
                  <>
                    <Link href="/account" className={styles.dropdownItem}><User size={16} /> My Account</Link>
                    <Link href="/orders" className={styles.dropdownItem}><Package size={16} /> Orders</Link>
                    <Link href="/wishlist" className={styles.dropdownItem}><Heart size={16} /> Wishlist</Link>
                    <Link href="" onClick={handleLogout} className={styles.dropdownItemx}> Logout</Link>
                  </>
                ) : (
                  <>
